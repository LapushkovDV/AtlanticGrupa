...
)) and Дата>=sertific.ddoc and Дата<=sertific.dend
and sertific.status = 1
;

// LapushkovDV
.create view vMarshrut as select
 MARAVT.NPADDR  //- адрес из маршрута
,MARREL.NAME    //- наименование из пункта разгрузки
,MARREL.NOMLICH //- КПП из пункта разгрузки
from MARREL, MARPUNKT, MARAVT
Where ((
              1109 == TTNDOC.WTABLE
and накладная_нрек == TTNDOC.CDOC
...
! формирование MPrnSoprNakl
...
#ifdef _DROGA
  recNakl.Class_OsnOtp       := Назван_накладной;
  if pos('ДО ', recNakl.Class_OsnOtp) > 0
    recNakl.Class_OsnOtp     := substr(recNakl.Class_OsnOtp, 1, pos('ДО ', recNakl.Class_OsnOtp)- 2);
#else
  ...
  // Штрих-код накладной
  if ( MemTblSopr.GetFirst KatSopr where (( накладная_нрек == KatSopr.nRec )) = tsOk )
  {
    // Сводная накладная
    ...
    {
      recNakl.BarCode := GenerateBarCodeEx(coKatSopr, накладная_нрек);
    }
  }
...
! Поставщик - 1
  ...
  recOrg.KatOrg_OKATO        := Поставщик_OKATO;
  recOrg.KatOrg_KBK          := Поставщик_KBK;
  recOrg.KatOrg_OGRN         := Поставщик_OGRN;
  recOrg.KatOrg_REGNO        := Поставщик_РегНом_в_ПФ;
  recOrg.KatOrg_REGNOS       := Поставщик_РегНом_Страхователя;
  recOrg.KatOrg_INSNA        := Поставщик_Орган_Исполн_Власти;
...
! Грузоотправитель - 2
  ...
  recOrg.KatOrg_OKATO        := Грузоотправитель_OKATO;
  recOrg.KatOrg_KBK          := Грузоотправитель_KBK;
  recOrg.KatOrg_OGRN         := Грузоотправитель_OGRN;
  recOrg.KatOrg_REGNO        := Грузоотправитель_РегНом_в_ПФ;
  recOrg.KatOrg_REGNOS       := Грузоотправитель_РегНом_Страхователя;
  recOrg.KatOrg_INSNA        := Грузоотправитель_Орган_Исполн_Власти;
...
! Грузополучатель - 3
  ...
  recOrg.PrOrg               := 3;
/* lapushkovdv
 тут меняем наименование, адрес и КПП и в FR изменим наименование грузополучателя на grpol.name и адрес на grpol.ur_addr
 Сначала заполним стандартом, а потом вьюхой
*/

  recOrg.KatOrg_Name         := oPrnSoprFun.GetOrgFullName(накладная_нрек, 2, 0); //Грузополучатель; тут ставим сразу как в коде далее будет oPrnSoprFun.GetOrgFullName(fr_dochead.KatSoprNRec, 2, 0);
  recOrg.KatOrg_KODPLAT      := ГрузополучательКПП;

  recOrg.KatOrg_UrAddr       := oPrnSoprFun.GetAddr(ТТН_ПунктР_Адрес, ТТН_ПунктР_Имя, Грузополучатель_Адрес);//Грузополучатель_ЮрАдрес; меняем так, чтобы с учетом настройки адрес разгрузки из пункта разгрузки

 if(vMarshrut.getfirst TTNDOC = tsOK)
 if(vMarshrut.getfirst MARPUNKT = tsOK)
 if(vMarshrut.getfirst MARREL = tsOK)
  {
    recOrg.KatOrg_Name    := if(vMarshrut.MARREL.NAME    <> '', vMarshrut.MARREL.NAME   , recOrg.KatOrg_Name  ); //- наименование из маршрута
    recOrg.KatOrg_KODPLAT := if(vMarshrut.MARREL.NOMLICH <> '', vMarshrut.MARREL.NOMLICH, recOrg.KatOrg_KODPLAT); //- КПП из маршрута
    if(vMarshrut.getfirst MARAVT = tsOK)
     {
      recOrg.KatOrg_UrAddr := if(vMarshrut.MARAVT.NPADDR <> '', vMarshrut.MARAVT.NPADDR, recOrg.KatOrg_UrAddr); //- адрес из маршрута
  ...
  recOrg.KatOrg_OKATO        := Грузополучатель_OKATO;
  recOrg.KatOrg_KBK          := Грузополучатель_KBK;
  recOrg.KatOrg_OGRN         := Грузополучатель_OGRN;
  recOrg.KatOrg_REGNO        := Грузополучатель_РегНом_в_ПФ;
  recOrg.KatOrg_REGNOS       := Грузополучатель_РегНом_Страхователя;
  recOrg.KatOrg_INSNA        := Грузополучатель_Орган_Исполн_Власти;
...
! Получатель - 4
  ...
  recOrg.KatOrg_OKATO        := Получатель_OKATO;
  recOrg.KatOrg_KBK          := Получатель_KBK;
  recOrg.KatOrg_OGRN         := Получатель_OGRN;
  recOrg.KatOrg_REGNO        := Получатель_РегНом_в_ПФ;
  recOrg.KatOrg_REGNOS       := Получатель_РегНом_Страхователя;
  recOrg.KatOrg_INSNA        := Получатель_Орган_Исполн_Власти;
...
! Плательщик - 5
  ...
  recOrg.KatOrg_OKATO        := Плательщик_OKATO;
  recOrg.KatOrg_KBK          := Плательщик_KBK;
  recOrg.KatOrg_OGRN         := Плательщик_OGRN;
  recOrg.KatOrg_REGNO        := Плательщик_РегНом_в_ПФ;
  recOrg.KatOrg_REGNOS       := Плательщик_РегНом_Страхователя;
  recOrg.KatOrg_INSNA        := Плательщик_Орган_Исполн_Власти;
...
! Заказчик_Плательщик - 6
  ...
  recOrg.KatOrg_OKATO        := Заказчик_Плательщик_OKATO;
  recOrg.KatOrg_KBK          := Заказчик_Плательщик_KBK;
  recOrg.KatOrg_OGRN         := Заказчик_Плательщик_OGRN;
  recOrg.KatOrg_REGNO        := Заказчик_Плательщик_РегНом_в_ПФ;
  recOrg.KatOrg_REGNOS       := Заказчик_Плательщик_РегНом_Страхователя;
  recOrg.KatOrg_INSNA        := Заказчик_Плательщик_Орган_Исполн_Власти;
...
! ТТН_АвтоОрг - 7
  ...
  recOrg.KatOrg_OKATO        := ТТН_АвтоОрг_OKATO;
  recOrg.KatOrg_KBK          := ТТН_АвтоОрг_KBK;
  recOrg.KatOrg_OGRN         := ТТН_АвтоОрг_OGRN;
  recOrg.KatOrg_REGNO        := ТТН_АвтоОрг_РегНом_в_ПФ;
  recOrg.KatOrg_REGNOS       := ТТН_АвтоОрг_РегНом_Страхователя;
  recOrg.KatOrg_INSNA        := ТТН_АвтоОрг_Орган_Исполн_Власти;
...
! ТТН_ПунктР_Орг - 8
  ...
  recOrg.KatOrg_OKATO        := ТТН_ПунктР_Орг_OKATO;
  recOrg.KatOrg_KBK          := ТТН_ПунктР_Орг_KBK;
  recOrg.KatOrg_OGRN         := ТТН_ПунктР_Орг_OGRN;
  recOrg.KatOrg_REGNO        := ТТН_ПунктР_Орг_РегНом_в_ПФ;
  recOrg.KatOrg_REGNOS       := ТТН_ПунктР_Орг_РегНом_Страхователя;
  recOrg.KatOrg_INSNA        := ТТН_ПунктР_Орг_Орган_Исполн_Власти;
...
! ТТН_ПунктП_Орг - 9
  ...
  recOrg.KatOrg_OKATO        := ТТН_ПунктП_Орг_OKATO;
  recOrg.KatOrg_KBK          := ТТН_ПунктП_Орг_KBK;
  recOrg.KatOrg_OGRN         := ТТН_ПунктП_Орг_OGRN;
  recOrg.KatOrg_REGNO        := ТТН_ПунктП_Орг_РегНом_в_ПФ;
  recOrg.KatOrg_REGNOS       := ТТН_ПунктП_Орг_РегНом_Страхователя;
  recOrg.KatOrg_INSNA        := ТТН_ПунктП_Орг_Орган_Исполн_Власти;
...
! СпПунктР_Орг - 10
  ...
  recOrg.KatOrg_OKATO    := СпТТН_ПунктП_Орг_OKATO;
  recOrg.KatOrg_KBK      := СпТТН_ПунктП_Орг_KBK;
  recOrg.KatOrg_OGRN     := СпТТН_ПунктП_Орг_OGRN;
  recOrg.KatOrg_REGNO    := СпТТН_ПунктП_Орг_РегНом_в_ПФ;
  recOrg.KatOrg_REGNOS   := СпТТН_ПунктП_Орг_РегНом_Страхователя;
  recOrg.KatOrg_INSNA    := СпТТН_ПунктП_Орг_Орган_Исполн_Власти;
