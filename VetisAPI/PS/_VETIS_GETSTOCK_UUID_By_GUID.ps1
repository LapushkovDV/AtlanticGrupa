...
$StockGUID = 'eedc5bfa-227c-4448-aa0d-d83582cbdb10'
    ...
    <apldef:submitApplicationRequest>
     <apldef:apiKey>{0}</apldef:apiKey>
      <apl:application>
        ...
        <apl:issueDate>2019-07-04T17:24:45+03:00</apl:issueDate>
        <apl:data>
     ...
     if ($currentStatus -eq 'REJECTED') 
      {
       $isContinue = $false
       $result = $null
       write-host 'Запрос отклонен'
       $GlobError = $XmlResponse.Envelope.Body.receiveApplicationResultResponse.application.errors.error.'#text'
      }
...
#    $stockEntryElementList.SelectNodes("//soap:Envelope/soap:Body/ns1:getStockEntryListResponse", $ns)
    ...
    # пошлда спецификация
    foreach($stockEntry in $stockEntryElementList.Envelope.Body.receiveApplicationResultResponse.application.result.getStockEntryByGuidResponse.stockEntry)
    {
    ...
    $item | Add-Member -type NoteProperty -Name 'REQUESTERROR' -Value $GlobError.ToString()        #REQUESTSTATUS 
    ...
    $item | Add-Member -type NoteProperty -Name 'гуид страны происхождения партии'   -value  $stockEntry.batch.origin.country.guid 
    $item | Add-Member -type NoteProperty -Name 'Глобальный идентификатор предприятия-производителя продукции.'    -value  $stockEntry.batch.origin.producer.enterprise.guid  
    $item | Add-Member -type NoteProperty -Name 'Роль предприятия-производителя продукции.'    -value  $stockEntry.batch.origin.producer.role
    $item | Add-Member -type NoteProperty -Name 'lowGradeCargo' -value $stockEntry.batch.lowGradeCargo.ToString()
...
#    $item | Add-Member -type NoteProperty -Name '' -value $stockEntry.Batch.ExpiryDate.FirstDate.day
 ...
 $ReportPath = $ReportPath+"StockLIst_TEST_GUID_"+$stockEntryElementList.Envelope.Body.receiveApplicationResultResponse.application.applicationId.ToString()+".xlsx"
